/*!
 * @license
 * TradingView Lightweight Charts v3.8.0-dev+202211241548
...
(function () {
    ...
    ***************************************************************************** */
    ...
    var BoxRenderer = /** @class */ (function () {
        function BoxRenderer() {
            this._private__data = null;
        }
        BoxRenderer.prototype._internal_setData = function (data) {
            this._private__data = data;
        };
        BoxRenderer.prototype.draw = function (ctx, pixelRatio, isHovered, hitTestData) {
            if (this._private__data === null) {
                return;
            }
            if (this._private__data._internal_visible === false) {
            ...
            if (this._private__data._internal_corners.length === 0) {
                var height = Math.ceil(this._private__data._internal_height * pixelRatio);
                var yLow = Math.round(this._private__data._internal_yLow * pixelRatio);
                if (yLow > height) {
                    return;
                }
                var yHigh = Math.round(this._private__data._internal_yHigh * pixelRatio);
                if (yHigh < 0) {
                    return;
                }
                var width = Math.ceil(this._private__data._internal_width * pixelRatio);
                var xLow = Math.round(this._private__data._internal_xLow * pixelRatio);
                if (xLow > width) {
                    return;
                }
                var xHigh = Math.round(this._private__data._internal_xHigh * pixelRatio);
                if (xHigh < 0) {
                    return;
                }
                corners = [
                ...
                for (var i = 0; i < this._private__data._internal_corners.length; ++i) {
                    corners.push({
                        x: Math.round(this._private__data._internal_corners[i].x * pixelRatio),
                        y: Math.round(this._private__data._internal_corners[i].y * pixelRatio),
                    });
                }
            }
            ctx.beginPath();
        ...
        BoxRenderer.prototype._private__hexToRgba = function (hex, opacity) {
            hex = hex.substring(1);
            if (hex.length === 3) {
    ...
    var SeriesBoxPaneView = /** @class */ (function () {
        ...
        SeriesBoxPaneView.prototype._internal_update = function () {
            this._private__invalidated = true;
        };
        SeriesBoxPaneView.prototype.renderer = function (height, width) {
            if (!this._internal__series.visible()) {
                return null;
            }
            if (this._private__invalidated) {
        ...
        return SeriesBoxPaneView;
    }());

    var CustomBoxPaneView = /** @class */ (function (_super) {
        __extends(CustomBoxPaneView, _super);
        function CustomBoxPaneView(series, box) {
            var _this = _super.call(this, series) || this;
            _this._private__box = box;
            return _this;
        }
        CustomBoxPaneView.prototype._internal__updateImpl = function (height, width) {
            var data = this._internal__boxRendererData;
            var boxOptions = this._private__box.options();
            data._internal_visible = false;
            if (!this._internal__series.visible()) {
                return;
            }
            ...
            if (boxOptions.corners.length === 0) {
                var yLow = this._private__box.yLowCoord();
                var yHigh = this._private__box.yHighCoord();
                if (yLow === null || yHigh === null) {
                    return;
                }
                var xLow = this._private__box.xLowCoord();
                var xHigh = this._private__box.xHighCoord();
                if (xLow === null || xHigh === null) {
                    return;
                }
                data._internal_xLow = xLow;
                data._internal_xHigh =
                    boxOptions.lateTime === 0
                ...
                for (var i = 0; i < boxOptions.corners.length; ++i) {
                    var x = this._private__box.xCoord(boxOptions.corners[i].time);
                    var y = this._private__box.yCoord(boxOptions.corners[i].price);
                    if (x === null || y === null) {
                        return;
                    }
        ...
        return CustomBoxPaneView;
    }(SeriesBoxPaneView));

    var CustomBox = /** @class */ (function () {
        function CustomBox(series, options) {
            this._private__series = series;
            this._private__options = options;
        ...
        CustomBox.prototype.applyOptions = function (options) {
            merge(this._private__options, options);
            this.update();
            this._private__series.model().lightUpdate();
        };
        CustomBox.prototype.options = function () {
            return this._private__options;
        };
        CustomBox.prototype.paneView = function () {
        ...
        // }
        CustomBox.prototype.update = function () {
            this._private__boxView._internal_update();
            // this._priceAxisView.update();
        };
        CustomBox.prototype.xLowCoord = function () {
            return this.xCoord(this._private__options.earlyTime);
        };
        CustomBox.prototype.xHighCoord = function () {
            return this.xCoord(this._private__options.lateTime);
        };
        CustomBox.prototype.yLowCoord = function () {
            // low Y coord = the high price (it is intentionally flipped)
            return this.yCoord(this._private__options.highPrice);
        };
        CustomBox.prototype.yHighCoord = function () {
            // high Y coord = the low price (it is intentionally flipped)
            return this.yCoord(this._private__options.lowPrice);
        };
        CustomBox.prototype.xCoord = function (time) {
            var series = this._private__series;
            var timeScale = series.model().timeScale();
            var timeIndex = timeScale.timeToIndex({ timestamp: time }, true);
            if (timeScale.isEmpty() || timeIndex === null) {
                return null;
            }
            return timeScale.indexToCoordinate(timeIndex);
        };
        CustomBox.prototype.yCoord = function (price) {
            var series = this._private__series;
            var priceScale = series.priceScale();
            var timeScale = series.model().timeScale();
            if (timeScale.isEmpty() || priceScale.isEmpty()) {
    ...
    var Series = /** @class */ (function (_super) {
        ...
        Series.prototype.createBox = function (options) {
            var result = new CustomBox(this, options);
            this._private__customBoxes.push(result);
            this.model().updateSource(this);
            return result;
        };
        Series.prototype.removeBox = function (box) {
            var index = this._private__customBoxes.indexOf(box);
            if (index !== -1) {
        ...
        Series.prototype.applyOptions = function (options) {
            ...
            if (targetPriceScaleId !== undefined &&
        ...
        Series.prototype.paneViews = function () {
            ...
            var boxViews = this._private__customBoxes.map(function (box) { return box.paneView(); });
        ...
        Series.prototype.updateAllViews = function () {
            ...
            for (var _e = 0, _f = this._private__customBoxes; _e < _f.length; _e++) {
                var customBox = _f[_e];
                customBox.update();
            }
            this._private__priceLineView._internal_update();
        ...
        Series.prototype.markerDataAtIndex = function (index) {
            var getValue = (this._private__seriesType === "Line" ||
        ...
        Series.prototype._private__autoscaleInfoImpl = function (startTimePoint, endTimePoint) {
            ...
            var plots = this._private__seriesType === "Line" ||
                this._private__seriesType === "Area" ||
                this._private__seriesType === "Baseline" ||
        ...
        Series.prototype._private__recalculateMarkers = function () {
            ...
            this._private__indexedMarkers = this._private__markers.map(function (marker, index) {
                ...
                var searchMode = timePointIndex < firstDataIndex
                    ? 1 /* NearestRight */
                    : -1 /* NearestLeft */;
    ...
    function checkBoxOptions(options) {
        if (options.corners.length === 0) {
            // eslint-disable-next-line @typescript-eslint/tslint/config
            assert(typeof options.lowPrice === "number", "the type of 'lowPrice' box's property must be a number, got '".concat(typeof options.lowPrice, "'"));
            // eslint-disable-next-line @typescript-eslint/tslint/config
            assert(typeof options.highPrice === "number", "the type of 'highPrice' box's property must be a number, got '".concat(typeof options.highPrice, "'"));
        }
        else {
    ...
    var Box = /** @class */ (function () {
        ...
        Box.prototype.applyOptions = function (options) {
            this._private__box.applyOptions(options);
        };
        Box.prototype.options = function () {
            return this._private__box.options();
        };
        Box.prototype._internal_box = function () {
            return this._private__box;
        };
        return Box;
    }());

    var boxOptionsDefaults = {
        corners: [],
        lowPrice: 0.0,
        highPrice: 0.0,
        earlyTime: 0,
    ...
    var SeriesApi = /** @class */ (function () {
        ...
        SeriesApi.prototype.coordinateToPrice = function (coordinate) {
            ...
            return this._internal__series
                .priceScale()
                .coordinateToPrice(coordinate, firstValue.value);
        };
        // eslint-disable-next-line complexity
        SeriesApi.prototype.barsInLogicalRange = function (range) {
            ...
            var barsBefore = dataFirstBarInRange === null ||
            ...
            if (dataFirstBarInRange !== null && dataLastBarInRange !== null) {
                result.from =
                    dataFirstBarInRange.time.businessDay ||
                        dataFirstBarInRange.time.timestamp;
                result.to =
                    dataLastBarInRange.time.businessDay ||
                        dataLastBarInRange.time.timestamp;
            }
            return result;
        };
        ...
        SeriesApi.prototype.createBox = function (options) {
            var strictOptions = merge(clone(boxOptionsDefaults), options);
            checkBoxOptions(strictOptions);
            var box = this._internal__series.createBox(strictOptions);
            return new Box(box);
        };
        SeriesApi.prototype.removeBox = function (box) {
            this._internal__series.removeBox(box._internal_box());
        };
